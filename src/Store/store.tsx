...
import addLocalStorage from "../Components/LocalStorage";

import { createContext } from "react";


interface ContextData{
  handleOpen:(key:any)=>void
  handleClose: () => void;
...
export const ContextProvider:React.FC<{children:any}>=({children})=>{
    ...
    //    }
      ...
      const handleUpdateGrid = useCallback(() => {
        ...
        if (addedColumns > 0) {
          ...
          for (let i = 0; i < addedColumns; i++) {
            ...
            for (let j = 0; j < gridRef.current.length; j++) {
                gridRef.current[j].splice(
                  position,
                  0,
                  null
                );
      ...
      return (
        <ContextCreator.Provider value={{handleOpen,handleChange,handleForm,handleClose,moveBlock,addItem,handleReset,blockRef,gridRef,open}} >
          {children}
